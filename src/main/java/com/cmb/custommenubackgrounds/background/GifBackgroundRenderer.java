...
public class GifBackgroundRenderer implements BackgroundRenderer {
    ...
    public GifBackgroundRenderer(Path file, ScaleMode scaleMode) {
        ...
        try {
            ...
            System.err.println("[CustomMenuBackgrounds] Failed to load GIF background '" + file + "': " + e.getMessage());
        }
    }
    ...
    private void advanceFrame(int index) {
        currentFrame = index;
        texture.upload(frames.get(index).image.getWidth() > 0 ? copyForUpload(frames.get(index)) : frames.get(index).image);
    }
