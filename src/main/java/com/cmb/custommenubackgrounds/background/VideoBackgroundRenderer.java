...
public class VideoBackgroundRenderer implements BackgroundRenderer {
    ...
    private final AtomicReference<NativeImage> pendingUpload = new AtomicReference<>();
    ...
    private void decodeLoop() {
        try {
            ...
            System.err.println("[CustomMenuBackgrounds] Video decode thread stopped: " + e.getMessage());
        }
    }
    ...
    public void tick(float deltaSeconds) {
        ...
        if (frameTimer >= scaledDuration) {
            frameTimer -= scaledDuration;
            NativeImage next = decodedFrames.poll();
            if (next != null) {
                pendingUpload.set(next);
                texture.upload(next);
            }
        }
    }
