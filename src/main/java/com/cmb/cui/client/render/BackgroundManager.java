...
public final class BackgroundManager {
    ...
    public void rebuild() {
        ...
        try {
            switch (type) {
                ...
                case VIDEO -> {
                    ...
                    try {
                        ...
                        System.err.println("[CUI] " + videoError.getMessage()
            ...
            System.err.println("[CUI] Failed to build background, falling back to vanilla: " + e.getMessage());
            active = null;
            activeType = BackgroundType.VANILLA;
        }
    }
        ...
        if (config.background != null && !config.background.isBlank()) {
            ...
            if (java.nio.file.Files.isRegularFile(candidate)) {
                try {
                    ...
                    System.err.println("[CUI] Fallback static background also failed: " + e.getMessage());
                }
            }
        }
        active = null;
    ...
    /** Called when entering a world, to free GPU/decoder resources. */
    public void onMenuClosed() {
        closeActive();
        activeType = BackgroundType.VANILLA;
        // Renderer will be lazily rebuilt from config next time any menu
        // screen renders, via onMenuOpened() - see ScreenPanoramaMixin.
    }

    /** Called when (re)entering any menu screen, so a fresh renderer is ready. */
